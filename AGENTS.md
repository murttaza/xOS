...
**The user-facing brand is `mOS`** — sourced from `src/lib/brand.ts` (`APP_NAME`). Installer productName, window titles, tray, manifest, and export filenames must all say mOS. Personal flourishes (Arabic signature/meem) render only for the account matching `VITE_OWNER_EMAIL` (optional env var); distributed builds stay neutral.
...
- **Electron**: `web-contents-created` denies window.open (external http(s) → shell) and non-app navigation; DevTools only when unpackaged; display-media capture requires renderer arming (`arm-audio-capture`) right before `getDisplayMedia`; updates download automatically but install only after a user-confirmed dialog; `verifyUpdateCodeSignature: true` (signing cert still TODO — see electron-builder.json5).
